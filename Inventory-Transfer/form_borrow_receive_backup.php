...
//                alert(store+'---------------'+location);
             ...
             $.post("api/lending_received",{tx_id:tx_id,store:store,location:location,round:round,inv_code:inv_code,amt_movein:amt_movein},function(data){
                 if(data='updated'){
...
function loadLocationByStore(round){
//    alert('aaaaaaaaaaa');
    var store = $('#store_receive'+round).val();
        ...
        <table class="tinput" style="width: 100%;border-collapse: collapse;border: 1px solid #CDCDCD;">
                <thead style="background-color: #E6EEEE;font-size: 8pt;">
                    <tr  style="width:100%">
                        ...
                        <th>จำนวนที่ได้รับ(จำนวนที่เหลือ)</th>
...
//                echo $sql1;
                ...
                <td style="text-align: center;">
                    ...
                    <input onchange="ifAmtNotDone('<? echo $i ?>','<? echo $row["inv_code"] ?>')" name="amt_movein<? echo $i ?>" id="amt_movein<? echo $i ?>" type="text"  ></td>
                <? }else{ ?>
                ...
                <td style="text-align: center;"  style="width:100%">
                    <table style="width:100%">
                        ...
                        <tr style="width:100%">
                            <td>
                    <select onchange="loadLocationByStore('<? echo $i ?>')" name="store_receive<? echo $i ?>" id="store_receive<? echo $i ?>"  >
                        <option value="" >--- เลือก Store ---</option>
                        <?
                
                $sel_store = " select * from inv_store where factory_code = '{$_SESSION['factory_code']}' ";
                $rs_store = $conn->queryTis620($sel_store);
                while($row_store = $conn->parseArray($rs_store)){

                ?>
                        <option <? echo $selected ?>  value="<? echo $row_store['code'] ?>"><? echo $row_store['nameth'] ?></option> 
                <? } ?>
                        </select>
                            </td>
                            <td>
                        <select class="fieldNotNull" name="loca_receive<? echo $i ?>" id="loca_receive<? echo $i ?>"  ></select>
                            </td>
                        </tr>
                        <? } ?>
            <? 
